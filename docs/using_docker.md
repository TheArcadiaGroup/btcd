...
With Docker you can easily set up *btcd* to run your Bitcoin full node. You can find the official *btcd* Docker images on Docker Hub [TheArcadiaGroup/firod](https://hub.docker.com/r/TheArcadiaGroup/firod). The Docker source file of this image is located at [Dockerfile](https://github.com/TheArcadiaGroup/firod/blob/master/Dockerfile).
...
services:
  btcd:
    container_name: btcd
    hostname: btcd
    image: TheArcadiaGroup/firod:latest
    restart: unless-stopped
    volumes:
      - btcd-data:/root/.btcd
    ports:
